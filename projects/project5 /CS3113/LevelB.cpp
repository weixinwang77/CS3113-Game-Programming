...
void LevelB::initialise()
{
    ...
    mGameState.spawnTimer        = 1.5f;
    mGameState.pickupSpawnTimer  = 12.0f;
    mGameState.blizzardIntensity = 0.0f;
    mGameState.copCaught         = false;
    mGameState.map               = nullptr;

    mGameState.bgTexture = LoadTexture("assets/game/desert.png");
    mGameState.roadTexture = LoadTexture("assets/game/road_vertical.png");

    mGameState.bgm = LoadMusicStream("assets/game/desert_music.mp3");
...
void LevelB::update(float deltaTime)
{
    ...
    if (mGameState.spawnTimer <= 0.0f)
    {
        ...
        mGameState.spawnTimer = GetRandomValue(10, 18) / 10.0f;
    }
...
void LevelB::render()
{
    ClearBackground(ColorFromHex("#c2a14d"));
