...
constexpr float ROAD_LEFT  = 300.0f;
constexpr float ROAD_RIGHT = 700.0f;
constexpr float ROAD_WIDTH = 400.0f;
constexpr float LANE_1     = 367.0f;
constexpr float LANE_2     = 500.0f;
constexpr float LANE_3     = 633.0f;
constexpr float PLAYER_Y   = 450.0f;
constexpr float ROAD_MIN_X = 320.0f;
constexpr float ROAD_MAX_X = 680.0f;
...
static void setupEnemy(Entity *e, const char *texture, AIType aiType, int speed, Direction dir, Vector2 scale, Vector2 collider)
{
...
void LevelA::initialise()
{
    ...
    mGameState.pickupSpawnTimer  = 10.0f;
    mGameState.blizzardIntensity = 0.0f;
    mGameState.copCaught         = false;
    mGameState.map               = nullptr;

    mGameState.bgTexture   = LoadTexture("assets/game/city_bg.png");
    mGameState.roadTexture = LoadTexture("assets/game/road_vertical.png");

    mGameState.bgm = LoadMusicStream("assets/game/city_music.mp3");
    ...
    for (int i = 0; i < 4; i++)
        setupEnemy(&mGameState.enemies[i], "assets/game/truck.png",
                   TRAFFIC, 220, UP, {80.0f, 80.0f}, {55.0f, 65.0f});

    for (int i = 4; i < 8; i++)
        setupEnemy(&mGameState.enemies[i], "assets/game/taxi.png",
                   TRAFFIC, 250, UP, {60.0f, 60.0f}, {40.0f, 50.0f});

    for (int i = 8; i < 12; i++)
        setupEnemy(&mGameState.enemies[i], "assets/game/ambulance.png",
                   TRAFFIC, 200, UP, {80.0f, 80.0f}, {55.0f, 65.0f});

    for (int i = 12; i < MAX_ENEMIES; i++)
        setupEnemy(&mGameState.enemies[i], "assets/game/police.png",
                   COP, 250, UP, {60.0f, 60.0f}, {40.0f, 50.0f});
...
void LevelA::update(float deltaTime)
{
    ...
    if (IsKeyDown(KEY_W) || IsKeyDown(KEY_UP))
        mGameState.scrollSpeed = BASE_SCROLL * 1.6f;
    ...
    if (mGameState.spawnTimer <= 0.0f)
    {
        ...
        mGameState.spawnTimer = GetRandomValue(12, 22) / 10.0f;
    }
    ...
    for (int i = 0; i < MAX_ENEMIES; i++)
    {
        ...
        if (mGameState.enemies[i].getPosition().y > 700.0f ||
            mGameState.enemies[i].getPosition().y < -200.0f)
    ...
    if (mGameState.pickupSpawnTimer <= 0.0f)
    {
        for (int i = 0; i < MAX_PICKUPS; i++)
        {
            if (!mGameState.pickups[i].isActive())
            {
                ...
                mGameState.pickups[i].setSpeed(200);
                mGameState.pickups[i].setMovement({0.0f, 1.0f});
                mGameState.pickups[i].activate();
                break;
            }
        }
        mGameState.pickupSpawnTimer = GetRandomValue(80, 150) / 10.0f;
    }
    ...
    for (int i = 0; i < MAX_PICKUPS; i++)
    {
        ...
        float py = mGameState.pickups[i].getPosition().y;
        py += 200.0f * deltaTime;
...
void LevelA::render()
{
    ClearBackground(ColorFromHex("#2d5a27"));
    ...
    for (float y = -bgH + bgScrollY; y < 620.0f; y += bgH)
    {
        ...
        DrawTexturePro(mGameState.bgTexture,
            {0, 0, bgW, bgH},
            {ROAD_RIGHT, y, 1000.0f - ROAD_RIGHT, bgH},
            {0, 0}, 0.0f, WHITE);
    }
